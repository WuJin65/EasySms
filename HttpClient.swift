...
class HttpClient: NSObject {
    ...
    func request(#URLString: String, get: Dictionary<String, String>?, onComplete: ((html: String, error: NSError?) -> ())?) {
        self.request(URL: NSURL(string: URLString)!, get: get, onComplete: onComplete)
    }
    
    var onComplete:((html:String, error:NSError?)->())?
    func request(URL url:NSURL, get:Dictionary<String,String>?, onComplete:((html:String, error:NSError?)->())?) {
        self.cancel();
        //设置回掉函数
